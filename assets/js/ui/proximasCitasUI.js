...
window.ProximasCitasUI = (function() {
    ...
    function renderizarProximasCitas(citas, container, callbacks) {
        if (!container) {
            console.error('❌ Container no proporcionado para renderizar próximas citas');
            return;
        }
    ...
    function asignarEventosBotones(container, callbacks) {
        if (!callbacks) {
            console.warn('⚠️ No se proporcionaron callbacks para los botones');
            return;
        }
    ...
    function renderizarPaginacion(actual, total, container, onCambiarPagina) {
        if (!container) {
            console.error('❌ Container no proporcionado para renderizar paginación');
            return;
        }
    ...
    function mostrarError(container, mensaje) {
        if (container) {
            container.innerHTML = '<p style="color: #e74c3c;">❌ Error: ' + escapeHtml(mensaje || 'No se pudo cargar las próximas citas.') + '</p>';
        }
    }

...
<?php elseif ($aa_clean_route['kind'] === AA_Canonical_Clean_Shell_Read_Route::RECORDS) : ?>
    ...
    <?php else : ?>
        ...
        <header class="mb-5">
            ...
            <div class="mt-3 flex items-start justify-between gap-3"><h1 class="text-2xl font-semibold tracking-tight text-gray-950"><?php echo esc_html($aa_clean_list['title']); ?></h1><details class="relative shrink-0"><summary class="aa-options-trigger-flat cursor-pointer list-none" aria-label="Opciones de lista">⋮</summary><div class="absolute right-0 z-20 mt-2 w-32 rounded-lg border border-gray-200 bg-white p-1 shadow-lg"><button class="block w-full rounded px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50" type="button" data-aa-clean-action="edit-list" data-aa-clean-list-id="<?php echo (int) $aa_clean_list['id']; ?>" data-aa-clean-title="<?php echo esc_attr($aa_clean_list['title']); ?>" data-aa-clean-details="<?php echo esc_attr((string) $aa_clean_list['details']); ?>">Editar</button><button class="block w-full rounded px-3 py-2 text-left text-sm text-red-700 hover:bg-red-50" type="button" data-aa-clean-action="delete-list" data-aa-clean-list-id="<?php echo (int) $aa_clean_list['id']; ?>" data-aa-clean-title="<?php echo esc_attr($aa_clean_list['title']); ?>">Eliminar</button></div></details></div>
...
<button class="fixed bottom-6 right-6 z-30 inline-flex h-14 w-14 items-center justify-center rounded-full bg-slate-900 text-3xl text-white shadow-lg transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2" type="button" data-aa-clean-action="<?php echo $aa_clean_route['kind'] === AA_Canonical_Clean_Shell_Read_Route::RECORDS ? 'create-record' : 'create-list'; ?>"<?php if ($aa_clean_route['kind'] === AA_Canonical_Clean_Shell_Read_Route::RECORDS) : ?> data-aa-clean-list-id="<?php echo (int) $aa_clean_route['container_id']; ?>"<?php endif; ?> aria-label="Crear">+</button>
<dialog id="aa-canonical-clean-dialog" class="w-[min(100%-2rem,32rem)] rounded-xl border border-gray-200 bg-white p-0 text-gray-900 shadow-2xl backdrop:bg-black/45"><form method="dialog" class="border-b border-gray-100 px-5 py-4"><div class="flex items-center justify-between gap-3"><h2 id="aa-canonical-clean-dialog-title" class="text-lg font-semibold"></h2><button class="rounded p-1 text-gray-500 hover:bg-gray-100" value="cancel" aria-label="Cerrar">×</button></div></form><div id="aa-canonical-clean-dialog-body" class="p-5"></div></dialog>
<script>window.AACanonicalCleanShell={ajaxUrl:<?php echo wp_json_encode(admin_url('admin-ajax.php')); ?>,nonce:<?php echo wp_json_encode(wp_create_nonce('aa_canonical_clean_shell')); ?>,rootUrl:<?php echo wp_json_encode($aa_clean_url(AA_Canonical_Clean_Shell_Read_Route::ROOT)); ?>};</script><script src="<?php echo aa_asset_url('includes/admin/ui/modules/canonical_clean_shell/canonical-clean-shell-mutations.js'); ?>" defer></script>

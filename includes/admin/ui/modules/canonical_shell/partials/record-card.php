...
$email_has_value = is_array($email_card) && ($email_card['kind'] ?? '') === 'value';
$email_has_error = is_array($email_card) && ($email_card['kind'] ?? '') === 'error';
...
$email_link_classes = 'aa-shell-record-email inline-flex items-center gap-1.5 text-sm text-gray-700 no-underline hover:underline m-0';
$dossier_action_enabled = is_array($dossier_action) && ($dossier_action['kind'] ?? '') === 'action' && !empty($dossier_action['enabled']);
$dossier_action_disabled = is_array($dossier_action) && ($dossier_action['kind'] ?? '') === 'action' && empty($dossier_action['enabled']);
$dossier_action_error = is_array($dossier_action) && ($dossier_action['kind'] ?? '') === 'error';
$dossier_reason = ($dossier_action_disabled && isset($dossier_action['reason'])) ? (string) $dossier_action['reason'] : '';
$dossier_title_attr = 'Expediente';
if ($dossier_reason === 'archive_disabled') {
    $dossier_title_attr = 'Activa Archivo en Ajustes para usar Expediente';
} elseif ($dossier_reason === 'dossier_retiring') {
    $dossier_title_attr = 'El expediente se está eliminando';
}
$dossier_btn_classes = 'aa-shell-dossier-btn inline-flex items-center px-3 py-1.5 text-xs font-semibold rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500';
$dossier_btn_enabled_classes = $dossier_btn_classes . ' text-indigo-700 bg-indigo-50 hover:bg-indigo-100';
$dossier_btn_disabled_classes = $dossier_btn_classes . ' text-gray-500 bg-gray-100 cursor-not-allowed opacity-80';
...
$email_svg = '<svg class="aa-shell-record-email__icon shrink-0" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>';
$panel_id = $card_record_id >= 1
    ? ('aa-shell-record-panel-' . $card_record_id)
    : ('aa-shell-record-panel-' . uniqid('', false));
$contact_block_shown = $whatsapp_has_value || $whatsapp_has_error || $phone_has_value || $phone_has_error || $email_has_value || $email_has_error;
?>
<?php if ($is_compact) : ?>
<li
...
>
    <div class="aa-shell-record-anchor relative">
        ...
        >
            ...
            <?php endif; ?>
            <?php if ($email_has_value) : ?>
                <p class="m-0<?php echo ($whatsapp_has_value || $whatsapp_has_error || $phone_has_value || $phone_has_error) ? ' mt-2' : ''; ?>">
                    <a
                        class="<?php echo esc_attr($email_link_classes); ?>"
                        href="<?php echo esc_attr((string) $email_card['href']); ?>"
                        aria-label="<?php echo esc_attr('Enviar correo: ' . (string) $email_card['display']); ?>"
                        title="<?php echo esc_attr('Enviar correo: ' . (string) $email_card['display']); ?>"
                    >
                        <?php echo $email_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup ?>
                        <span><?php echo esc_html((string) $email_card['display']); ?></span>
                    </a>
                </p>
            <?php elseif ($email_has_error) : ?>
                <p class="aa-shell-record-email-error text-sm text-amber-800 m-0<?php echo ($whatsapp_has_value || $whatsapp_has_error || $phone_has_value || $phone_has_error) ? ' mt-2' : ''; ?>" role="status">
            ...
            <?php endif; ?>
            <?php if ($dossier_action_enabled || $dossier_action_disabled || $dossier_action_error) : ?>
                <div class="aa-shell-record-capability-actions mt-3 pt-2 border-t border-gray-100">
                    <?php if ($dossier_action_enabled && $card_record_id >= 1) : ?>
                        <button
                            type="button"
                            class="<?php echo esc_attr($dossier_btn_enabled_classes); ?>"
                            data-aa-dossier-open="<?php echo esc_attr((string) $card_record_id); ?>"
                            aria-label="<?php echo esc_attr('Expediente: ' . $card_title); ?>"
                            title="<?php echo esc_attr($dossier_title_attr); ?>"
                        >
                            Expediente
                        </button>
                    <?php elseif ($dossier_action_disabled) : ?>
                        <button
                            type="button"
                            class="<?php echo esc_attr($dossier_btn_disabled_classes); ?>"
                            disabled
                            aria-disabled="true"
                            aria-label="<?php echo esc_attr($dossier_title_attr); ?>"
                            title="<?php echo esc_attr($dossier_title_attr); ?>"
                        >
                            Expediente
                        </button>
                    <?php else : ?>
                        <p class="aa-shell-record-dossier-error text-sm text-amber-800 m-0" role="status">
                            No se pudo cargar Expediente.
                        </p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
...
<li>
    <article class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 h-full flex flex-col">
        ...
        <?php elseif ($email_has_error) : ?>
            <p class="aa-shell-record-email-error mt-2 text-sm text-amber-800 m-0" role="status">
                No se pudo cargar el correo.
            </p>
        <?php endif; ?>
        <?php if ($has_details_text) : ?>

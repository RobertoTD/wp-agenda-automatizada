/**
 * Módulo whatsapp del formulario de registro del shell.
...
(function (global) {
    ...
    function ensureNodes() {
        if (!wrap) {
            wrap = document.getElementById('aa-shell-record-whatsapp-field');
        }
        if (!countrySelect) {
            countrySelect = document.getElementById('aa-shell-record-whatsapp-country');
        }
        if (!input) {
            input = document.getElementById('aa-shell-record-whatsapp');
        }
        if (!helpEl) {
            helpEl = document.getElementById('aa-shell-record-whatsapp-help');
        }
        if (!errorEl) {
            errorEl = document.getElementById('aa-shell-record-whatsapp-error');
        }
        if (!unavailableEl) {
            unavailableEl = document.getElementById('aa-shell-record-whatsapp-unavailable');
        }
    }
    ...
    function collect(formData) {
        ...
        formData.append('whatsapp', '+' + countrySelect.value + national);
    }

    function handleError(code, message) {
        // Mismos códigos que AA_Canonical_Phone_Normalizer (helper E.164 compartido).
        var whatsappCodes = {
            invalid_phone: true,
            phone_invalid_length: true,
            phone_unsupported_country: true
        };
        if (!whatsappCodes[code]) {
            return false;
        }
        ensureNodes();

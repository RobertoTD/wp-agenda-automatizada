<?php
/**
 * Ignore Learning Recommendation Use Case — posponer/degradar ("Ahora no") desde lista 1.
 */
...
final class IgnoreLearningRecommendationUseCase {
    ...
    public function execute(string $recommendation_key): array {
        ...
        if ((int) ($evaluated['effective_list'] ?? 0) !== AA_Learning_Visibility_Policy::LIST_PRIMARY) {
            return [
                'success' => false,
                'error' => [
                    'code' => 'not_in_primary_list',
                    'message' => 'Solo puedes posponer recomendaciones principales con "Ahora no".',
                ],
            ];
        }

        if (!empty($evaluated['is_ignored'])) {
            return [
                'success' => false,
                'error' => [
                    'code' => 'already_deferred',
                    'message' => 'Esta recomendación ya fue pospuesta.',
                ],
            ];
        }

        $now = LearningRecommendationEvaluator::resolve_now();
        $row = LearningRecommendationStateRepository::mark_ignored($key, $now);

<?php
/**
 * Learning State Migration Lifecycle — migración controlada legacy → Tasks común (MC13O-F2).
 *
 * Ejecuta MigrateLearningRecommendationStateToTaskStateUseCase en admin_init
 * prioridad 21, después del seed del catálogo Learning (prioridad 20).
 *
...
require_once dirname(__DIR__, 2) . '/application/tasks/MigrateLearningRecommendationStateToTaskStateUseCase.php';
require_once __DIR__ . '/LearningCatalogSeedLifecycle.php';

final class AA_Learning_State_Migration_Lifecycle {

    public const MIGRATION_VERSION = '1';

    public const OPTION_MIGRATION_VERSION = 'aa_learning_state_migration_version';

    public const OPTION_LAST_ERROR = 'aa_learning_state_migration_last_error';

    public const OPTION_LAST_RUN_AT = 'aa_learning_state_migration_last_run_at';

    public const LOCK_KEY = 'aa_learning_state_migration_lock';
    ...
    public static function register(string $main_plugin_file): void {
        add_action('admin_init', [__CLASS__, 'maybe_migrate'], 21);
    }

    /**
     * @internal Acceptance tests only.
     *
     * @param callable|null $executor Debe devolver el payload de MigrateLearningRecommendationStateToTaskStateUseCase::execute().
     */
    ...
    public static function maybe_migrate(): void {
        ...
        try {
            ...
            if (empty($result['success'])) {
                $message = (string) ($result['error']['message'] ?? 'learning state migration failed');
                error_log('[AA_Learning_State_Migration_Lifecycle] ' . $message);
            ...
            error_log('[AA_Learning_State_Migration_Lifecycle] Migration failed: ' . $exception->getMessage());
    ...
    private static function run_migration(): array {
        ...
        return (new MigrateLearningRecommendationStateToTaskStateUseCase())->execute();
    }

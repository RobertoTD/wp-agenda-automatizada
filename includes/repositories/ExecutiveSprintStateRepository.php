<?php
/**
 * Executive Sprint State Repository — persistencia de sprint ejecutivo en user_meta (MC4).
 *
...
require_once dirname(__DIR__) . '/domain/executive/class-aa-executive-sprint-policy.php';

final class ExecutiveSprintStateRepository {

    public const META_KEY = 'aa_executive_sprint_v1';
    ...
    public static function find_for_user(int $user_id): array {
        if ($user_id < 1) {
            return AA_Executive_Sprint_Policy::empty_state();
        }

        $raw = self::read_raw($user_id);

        return AA_Executive_Sprint_Policy::sanitize($raw);
    }
    ...
    public static function save_for_user(int $user_id, array $state): bool {
        ...
        $sanitized = AA_Executive_Sprint_Policy::sanitize($state);

        if ($sanitized === []) {
            return self::clear_for_user($user_id);
        }

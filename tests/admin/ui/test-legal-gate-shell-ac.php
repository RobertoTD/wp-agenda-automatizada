...
ac_assert('router loads GetLegalGateStatusUseCase', strpos($router, 'GetLegalGateStatusUseCase') !== false);
ac_assert('router branches away from layout when not ready', strpos($router, "legal-gate/index.php") !== false);
ac_assert('router keeps manage_options for shell', strpos($router, "current_user_can('manage_options')") !== false);
ac_assert('router still loads layout for ready', strpos($router, 'shared/layout.php') !== false);
ac_assert(
...
ac_assert('gate has checkbox', strpos($gate, 'aa-legal-gate-consent') !== false);
...
ac_assert('js enables accept only when checked', strpos($js, 'acceptBtn.disabled = submitting || !consent.checked') !== false);
...
ac_assert('js retry uses statusAction', strpos($js, 'cfg.statusAction') !== false);

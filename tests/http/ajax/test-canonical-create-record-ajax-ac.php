<?php
/**
 * AC Test — CanonicalCreateRecordAjax (SB1-5B2).
 *
 * Ejecutar: php tests/http/ajax/test-canonical-create-record-ajax-ac.php
 */
...
$ajax_file = $plugin_root . '/includes/http/ajax/CanonicalCreateRecordAjax.php';
...
ac_assert('Action constante', strpos($ajax_src, "ACTION = 'aa_create_canonical_record'") !== false);
ac_assert('Nonce específico', strpos($ajax_src, "NONCE_ACTION = 'aa_create_canonical_record'") !== false);
...
ac_assert('Bootstrap registra', strpos($boot_src, 'CanonicalCreateRecordAjax::register()') !== false);
...
function aa_run_create_record_ajax(array $post): ?array {
    $_POST = $post;
    $GLOBALS['aa_test_json'] = null;
    try {
        CanonicalCreateRecordAjax::handle();
...
function aa_base_record_post(array $over = []): array {
    return array_merge([
        'nonce' => 'good-nonce',
        'family_key' => 'finance',
        'container_id' => '1',
...
$r = aa_run_create_record_ajax(aa_base_record_post(['title' => ['x']]));
...
$r = aa_run_create_record_ajax(aa_base_record_post(['container_id' => '12abc']));
ac_assert('container_id dirty → invalid_container_id', ($r['data']['code'] ?? '') === 'invalid_container_id');

$r = aa_run_create_record_ajax(aa_base_record_post(['container_id' => '1.5']));
ac_assert('container_id decimal → invalid_container_id', ($r['data']['code'] ?? '') === 'invalid_container_id');

$GLOBALS['aa_test_caps'] = [];
$r = aa_run_create_record_ajax(aa_base_record_post(['family_key' => 'archive']));
...
$r = aa_run_create_record_ajax(aa_base_record_post(['title' => '   ']));
ac_assert('Blank title → invalid_title', ($r['data']['code'] ?? '') === 'invalid_title');

$r = aa_run_create_record_ajax(aa_base_record_post(['title' => str_repeat('x', 201)]));
...
$r = aa_run_create_record_ajax(aa_base_record_post(['container_id' => '99']));
...
ac_assert('Uncertain message safe', strpos((string) ($r['data']['message'] ?? ''), 'Revisa la lista') !== false);

AA_Canonical_Write_Binding_Bootstrap::$mode = 'fixture';
$r = aa_run_create_record_ajax(aa_base_record_post(['details' => '']));

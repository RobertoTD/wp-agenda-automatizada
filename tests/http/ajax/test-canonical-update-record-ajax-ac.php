<?php
/**
 * AC Test — CanonicalUpdateRecordAjax (SB1-5B3).
 *
 * Ejecutar: php tests/http/ajax/test-canonical-update-record-ajax-ac.php
 */
...
ac_assert('Action constante', strpos($ajax_src, "ACTION = 'aa_update_canonical_record'") !== false);
ac_assert('Nonce específico', strpos($ajax_src, "NONCE_ACTION = 'aa_update_canonical_record'") !== false);
...
ac_assert('Sin delete', strpos($ajax_src, 'delete') === false);
ac_assert('Shell modal unificado', strpos($shell_src, 'AA_CANONICAL_SHELL_RECORD_FORM') !== false
    && strpos($shell_src, 'canonical-shell-record-form.js') !== false
    && strpos($shell_src, 'updateAction') !== false);
ac_assert('Card Editar', strpos($card_src, 'aa-shell-edit-record-btn') !== false
    && strpos($card_src, 'data-aa-record') !== false
    && strpos($card_src, 'Editar') !== false);
ac_assert('JS modos create/update', strpos($js_src, 'MODE_UPDATE') !== false
    && strpos($js_src, 'updateAction') !== false
    && strpos($js_src, 'createAction') !== false);
ac_assert('Sin optimistic lock', strpos($ajax_src, 'expected_updated_at') === false
    && strpos($js_src, 'expected_updated_at') === false);
...
final class AA_Canonical_Write_Binding_Bootstrap {
    ...
    public static function register_productive(AA_Canonical_Write_Binding_Registry $registry, $repository = null): void {
        ...
        if (self::$mode === 'uncertain') {
            $adapter->uncertain_operation = 'update_record';
        }
...
function aa_run_update_record_ajax(array $post): ?array {
    $_POST = $post;
    $GLOBALS['aa_test_json'] = null;
    try {
        CanonicalUpdateRecordAjax::handle();
...
$r = aa_run_update_record_ajax(aa_base_update_post(['title' => ['x']]));
...
$r = aa_run_update_record_ajax(aa_base_update_post(['container_id' => '12abc']));
ac_assert('container_id dirty → invalid_container_id', ($r['data']['code'] ?? '') === 'invalid_container_id');

$r = aa_run_update_record_ajax(aa_base_update_post(['record_id' => '0']));
ac_assert('record_id 0 → invalid_record_id', ($r['data']['code'] ?? '') === 'invalid_record_id');

$r = aa_run_update_record_ajax(aa_base_update_post(['record_id' => '12abc']));
ac_assert('record_id dirty → invalid_record_id', ($r['data']['code'] ?? '') === 'invalid_record_id');

$r = aa_run_update_record_ajax(aa_base_update_post(['record_id' => '1.5']));
ac_assert('record_id decimal → invalid_record_id', ($r['data']['code'] ?? '') === 'invalid_record_id');

$GLOBALS['aa_test_caps'] = [];
$r = aa_run_update_record_ajax(aa_base_update_post(['family_key' => 'archive']));
...
$r = aa_run_update_record_ajax(aa_base_update_post(['title' => '   ']));
ac_assert('Blank title → invalid_title', ($r['data']['code'] ?? '') === 'invalid_title');

$r = aa_run_update_record_ajax(aa_base_update_post(['title' => str_repeat('x', 201)]));
...
$r = aa_run_update_record_ajax(aa_base_update_post(['container_id' => '99']));
ac_assert('Missing container → container_not_found', ($r['data']['code'] ?? '') === 'container_not_found');

$r = aa_run_update_record_ajax(aa_base_update_post(['record_id' => '999']));
ac_assert('Missing record → record_not_found', ($r['data']['code'] ?? '') === 'record_not_found');

$r = aa_run_update_record_ajax(aa_base_update_post(['container_id' => '2', 'record_id' => '10']));
...
ac_assert('Uncertain message safe', strpos((string) ($r['data']['message'] ?? ''), 'Revisa el registro') !== false);
ac_assert('Uncertain sin SQL', stripos((string) ($r['data']['message'] ?? ''), 'sql') === false);

AA_Canonical_Write_Binding_Bootstrap::$mode = 'fixture';
$r = aa_run_update_record_ajax(aa_base_update_post(['details' => '']));
...
ac_assert(
    ...
    && strpos($r['data']['redirect_url'], 'container_id=1') !== false

<?php
/**
 * AC Test — CanonicalCreateContainerAjax (SB1-5B1).
 *
 * Ejecutar: php tests/http/ajax/test-canonical-create-container-ajax-ac.php
 */
...
$ajax_file = $plugin_root . '/includes/http/ajax/CanonicalCreateContainerAjax.php';
...
ac_assert('Action constante', strpos($ajax_src, "ACTION = 'aa_create_canonical_container'") !== false);
ac_assert('Nonce específico', strpos($ajax_src, "NONCE_ACTION = 'aa_create_canonical_container'") !== false);
...
ac_assert('Bootstrap registra', strpos($boot_src, 'CanonicalCreateContainerAjax::register()') !== false);
...
ac_assert('UseCase create', strpos($ajax_src, 'WriteCanonicalShellContainerUseCase') !== false);
...
function aa_run_create_ajax(array $post): ?array {
    $_POST = $post;
    $GLOBALS['aa_test_json'] = null;
    try {
        CanonicalCreateContainerAjax::handle();
...
function aa_base_post(array $over = []): array {
    return array_merge([
        'nonce' => 'good-nonce',
        'family_key' => 'finance',
        'title' => 'Lista prueba',
...
$r = aa_run_create_ajax(aa_base_post());
...
$r = aa_run_create_ajax(aa_base_post(['title' => '   ']));
ac_assert('Blank title → invalid_title', ($r['data']['code'] ?? '') === 'invalid_title');

$r = aa_run_create_ajax(aa_base_post(['title' => str_repeat('x', 201)]));
...
$r = aa_run_create_ajax(aa_base_post(['details' => '']));
...
ac_assert(
    'Create: POST WP-slashed ["amount"] no invalid_payload',
...
ac_assert(
    'Create: scope slashed + selection [] confirmed',
    ($r['success'] ?? false) === true && ($r['data']['status'] ?? '') === 'confirmed'
);
...
ac_assert(
    'Create: ambos [] confirmed (explícito vacío)',
    ($r['success'] ?? false) === true && ($r['data']['status'] ?? '') === 'confirmed'
);
...
ac_assert(
    'Create: solo scope → invalid_payload',
    ($r['data']['code'] ?? '') === 'invalid_payload'
);

CanonicalCreateContainerAjax::register();
ac_assert('Register hook', in_array('wp_ajax_aa_create_canonical_container', $GLOBALS['aa_test_actions'], true));

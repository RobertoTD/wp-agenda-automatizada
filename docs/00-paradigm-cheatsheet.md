...
- `auth` (C2 agenda magic-link): GET `admin-post` `aa_agenda_access` (bridge HTML, no consume) → POST `aa_agenda_access_consume` → `ConsumeAgendaAccessTokenUseCase` (1× HMAC `/agenda/access/consume`); dominio `AA_Agenda_Access_Token_Format` / `User_Policy` / `Redirect_Policy`; infra `AA_Agenda_Access_Backend_Client` + `Session_Issuer` (remember 30d, filtro `auth_cookie_expiration` try/finally); handlers en `infrastructure/auth/AgendaAccessHandlers.php`. Solo tenants HMAC (`aa_client_secret`); sin `/agenda/access/request` aún.

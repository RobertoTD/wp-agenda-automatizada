...
- `canonical`: núcleo canónico de producto (Ciclo 1); dominio en `includes/domain/canonical/` (`AA_Canonical_Key`, `AA_Family_Definition`, `AA_Variant_Definition`, `AA_Canonical_Registry` instanciable y sellable); infraestructura en `includes/infrastructure/canonical/` (`AA_Canonical_Core_Bootstrap` para composición y shared instance de `finance.general`). Sin persistencia, hooks ni UI.

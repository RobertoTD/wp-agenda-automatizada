...
> **Canon DEO — fuente autoritativa:** `docs/04-canonical-constitution.md` (persistencia canónica universal). **Capacidades canónicas (asignación, configuración, activación, valores, `amount`, paradigma `images`):** `docs/05-canonical-capabilities.md` — normativo desde D0; `images` §12 (IMG-0). **Estado operativo:** `DB_VERSION=30`; repertorio `aa_canonical_family_capabilities`; config/escritura/lectura amount; selección por lista; `amount` ready; tablas físicas `images`/ops/purge (IMG-1) + credenciales de admisión en ops (IMG-3a) + inventario durable de retiro (IMG-5 inc. 2) + intención HMAC/cancelación (IMG-5 inc. 3) + checkpoints de retiro de lista (IMG-5 inc. 4); helper de consumo `AA_Installation_Storage_Usage`; attach canónico Application/AJAX (IMG-3b); lectura por lote + sign-read (IMG-4, `aa_sign_canonical_record_image_read`); retiro de **un registro** y de **una lista** vía mandatos (`aa_delete_canonical_record` / `aa_delete_canonical_container`); **`images` registrada not-ready — sin seeds/UI/galería de producto**. Lo que sigue bajo `canonical` es el inventario operativo de la implementación actual (familia canónica `finance` ≠ módulo legacy). No define por sí mismo el contrato universal ni sustituye la constitución. El plan temporal —ruta PCU y CAP-*— está en `docs/plans/shell-canonical-base-v1.md`. El shell de acceso administrativo/legal (`ResolveShellAccessUseCase`) es un concepto distinto.

> **Ruta PCU (estado):** PCU-0…PCU-5B y SB1-5B* completadas; C1a–A1b amount operativo; **LEGACY-X Finance completada** (DB 24); **selección por lista / repertorio** (`DB_VERSION=25`, decisión 31); **IMG-0** paradigma `images` documental (decisión 32); **IMG-1** schema+catálogo not-ready (`DB_VERSION=26`, decisión 33); **IMG-3a** consumo compartido + persistencia admisión (`DB_VERSION=27`, decisión 34); **IMG-3b** attach canónico Application/AJAX (decisión 35); **IMG-4** lectura/sign-read (decisión 36); **IMG-5** inc. 1 HMAC mandatos (decisión 37); **IMG-5** inc. 2 captura durable (`DB_VERSION=28`, decisión 38); **IMG-5** inc. 3 retiro de un registro (`DB_VERSION=29`, decisión 39); **IMG-5** inc. 4 retiro de lista (`DB_VERSION=30`, decisión 40). Shell familias → `aa_canonical_*` + capabilities. Distinguir: familia `finance` (canónica) ≠ módulo legacy retirado. Rollback DB22 histórico: `docs/plans/canonical-shell-db22-rollback.md`.

- `canonical`: implementación operativa del contexto canónico (familia `finance` canónica; **Finance legacy retirado en LEGACY-X** — sin `application/finance`, `aa_finance_*`, `module=canonical` ni `Shell_Url_Policy`). Dominio en `includes/domain/canonical/` (`AA_Canonical_Key`, `AA_Canonical_Family_Definition`, `AA_Canonical_Registry`). Aplicación en `includes/application/canonical/` (`ResolveCanonicalRouteUseCase`; cadena read SB1-2A/3A y write SB1-5A1; **PCU-5A** enablement: `SetCanonicalFamilyEnabled*` / `ReadCanonicalFamilyEnablementUseCase` / ports/snapshots/excepciones; **capabilities** config/escritura/lectura amount + contributors de página de registros; **IMG-3b/IMG-4** attach + contributor images + `aa_sign_canonical_record_image_read`; **IMG-5 inc. 2** `CaptureCanonicalPurgeInventoryUseCase`; **IMG-5 inc. 3** `RetireCanonicalRecordUseCase`; **IMG-5 inc. 4** `RetireCanonicalContainerUseCase`). Infraestructura en `includes/infrastructure/canonical/` (`AA_Canonical_Core_Bootstrap` familias `finance`+`archive`+`catalog`+`contact`; `AA_Canonical_Family_Provisioner` + catalog lifecycle `CATALOG_VERSION=2`; enablement store/nav; read/write binding bootstraps → Relational; **sin** adaptador Finance legacy; registries fail-on-duplicate) e `includes/infrastructure/wp/` (`AA_Canonical_Shell_Base_Url_Policy`; `AA_Canonical_Access_Policy`; `AA_Canonical_Schema` + capabilities DDL con **`DB_VERSION=30`**; `AA_Schema::retire_legacy_finance_tables()` DROP ordenado `aa_finance_records`→`aa_finance_containers`; dominio capacidades + `AA_Canonical_Amount_Normalizer`; repos/effects/Ops/lifecycle; Relational read/write adapters). Repositorios canónicos en `includes/repositories/` (`CanonicalRelationalRepository`, amount/capability repos; **IMG-3a** `CanonicalRecordImagesRepository` + `CanonicalImageUploadOperationsRepository`; **IMG-5 inc. 2** `CanonicalPurgeRunsRepository` + `CanonicalPurgeInventoryItemsRepository`). Consumo instalación: `includes/application/storage/AA_Installation_Storage_Usage.php`. Transporte AJAX canónico: `CanonicalFamilyEnabledAjax`, `CanonicalShellWriteAjaxSupport`/`Rejection`, seis endpoints universales create/update/delete container|record (`aa_delete_canonical_record` / `aa_delete_canonical_container` → retiro por mandatos). UI shell: `includes/admin/ui/modules/canonical_shell/` (`module=canonical_shell`); Settings enablement fuera de `options.php`; sidebar «Listas»; FAB/modales shell; gate `family_disabled`; layout `canonical-layout.php`. Ruta antigua `module=canonical` → 404 (módulo no encontrado). Amount UI vía capability contributors (A1b). Pendiente CAP: retiro de una imagen / clics UI Continuar del inc. 4 / Continuar UI y C de red del inc. 3 / activación `images` (`is_ready`); totalización si se autoriza.

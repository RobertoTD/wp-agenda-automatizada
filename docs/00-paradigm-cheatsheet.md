...
- `canonical`: núcleo canónico de producto (Ciclos 1, 2A, 3A1, 3A2, 3B1, 3B2a, 3B2b, 3C1, 3C2, 3D1, 3D2, 3D3A y 3D3B); dominio en `includes/domain/canonical/` (`AA_Canonical_Key`, `AA_Canonical_Family_Definition`, `AA_Canonical_Variant_Definition`, `AA_Canonical_Registry` instanciable y sellable); aplicación en `includes/application/canonical/` (`ResolveCanonicalRouteUseCase` para validación y resolución de ruta pura sin WordPress) y en `includes/application/finance/` (`CreateFinanceContainerUseCase`, `GetFinanceContainerUseCase`, `ListFinanceContainersUseCase`, `DeleteFinanceContainerUseCase`, `CreateFinanceRecordUseCase`, `GetFinanceRecordUseCase`, `ListFinanceRecordsUseCase`, `DeleteFinanceRecordUseCase` y `FinanceUseCaseSupport` para validación UTF-8, envelopes, normalización decimal de amount hasta 17 dígitos enteros y 2 decimales, verificación contextual anti-leak, proyección agregada de `amount_total` en contenedores y paginación fija de 15); infraestructura en `includes/infrastructure/canonical/` (`AA_Canonical_Core_Bootstrap` para composición y shared instance de `finance.general`) e `includes/infrastructure/wp/` (`AA_Canonical_Shell_Url_Policy` para URLs seguras allowlisted; `AA_Canonical_Access_Policy` para autorización neutral compartida de familias canónicas; `AA_Finance_Schema` en `FinanceSchema.php` para schema físico y FK de `aa_finance_containers` y `aa_finance_records` con `DB_VERSION=19`); repositorios SQL puro en `includes/repositories/` (`FinanceContainerRepository` y `FinanceRecordRepository` para CRUD básico, paginación fija de 15, suma `sum_amounts_by_container`, suma agregada en lote `sum_amounts_by_container_ids` y cascada referencial); transporte HTTP/AJAX en `includes/http/ajax/FinanceAjaxSupport.php` (soporte compartido para `aa_finance_nonce`, autorización `AA_Canonical_Access_Policy`, bootstrap y mapeo HTTP fail-closed), `includes/http/ajax/FinanceContainersAjax.php` (`aa_list_finance_containers`, `aa_create_finance_container`, `aa_get_finance_container`, `aa_delete_finance_container`) e `includes/http/ajax/FinanceRecordsAjax.php` (`aa_list_finance_records`, `aa_create_finance_record`, `aa_get_finance_record`, `aa_delete_finance_record` con `container_id` obligatorio, `record_id` unificado, solo `variant_key`, preservación estricta de `amount` y sin `nopriv`); UI en `includes/admin/ui/modules/canonical/` (dispatcher por mapa explícito y `_fallback.php` neutral) con submódulo `finance/` (`finance/index.php` con configuración `window.AA_FINANCE_DATA` aislada publicando `listContainers`, `createContainer`, `listRecords` y `createRecord`; `finance-records-module.js` como controlador de lectura paginada vía `aa_list_finance_records` con amounts `string|null`, estados RECORDS_*, callback retrocompatible `onAuthoritativeLoadSettled` y API `AA_FinanceRecords.createController`; `finance-record-create-module.js` como controlador de creación vía `aa_create_finance_record` con amount bruto sin parseo JS, snapshot inmutable, revisión incierta con `reviewToken`/`pendingRecordReview` en orquestador, refresh autoritativo de página 1 tras éxito confirmado y API `AA_FinanceRecordCreate.createController`; `finance-module.js` como orquestador con navegación interna CONTAINER_LIST/RECORDS_DETAIL sin cambio de URL, botón «Ver registros», botón «Nueva entrada» solo en detalle, preservación de snapshot del listado, degradación segura si falta factory/markup, máquina de creación de contenedores IDLE/EDITING/SUBMITTING/FIELD_REJECTED/BLOCKED_REJECTED/CONFIRMED/UNCERTAIN/REVIEWING_UNCERTAIN/DRAFT_REVIEWED, máquina de creación de registros separada, modal accesible, snapshot inmutable, timer de foco cancelable, foco a `#aa-finance-status` tras Cerrar y revisar, cierre unificado de BLOCKED_REJECTED a IDLE, focus trap con visibilidad real, prevención de doble submit y sincronización con página 1 de lectura) y layout ligero `includes/admin/ui/shared/canonical-layout.php`. Transporte vía `module=canonical&family=X[&variant=Y]`. Sin modales de edición/eliminación de contenedores ni registros.

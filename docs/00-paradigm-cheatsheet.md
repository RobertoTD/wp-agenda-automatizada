...
> **Canon DEO — fuente autoritativa:** `docs/04-canonical-constitution.md` (persistencia canónica universal). **Estado operativo:** `DB_VERSION=37`; repertorio y selección por lista de capabilities (`amount`, `images`, `phone`, `whatsapp`, `email`) y la solution independiente `contact_dossier`, aplicada por lista de Contactos y desactivada por defecto. La relación `contacto → Archivo` es recurso tipado de esa solution; `dossier` ya no figura en capabilities. El shell administrativo y el futuro runtime público son consumidores distintos del mismo núcleo horizontal.

> **Solutions canónicas:** `docs/06-canonical-solutions.md` define composiciones instalables de producto, distintas de capabilities y de la persistencia base. `contact_dossier` es la primera solution normativa y C3 completa su integración sin backfill ni fallback.

> **Ruta PCU (estado):** PCU-0…PCU-5B y SB1-5B* completadas; C1a–A1b amount operativo; **LEGACY-X Finance completada** (DB 24); **selección por lista / repertorio** (`DB_VERSION=25`, decisión 31); **IMG-0** paradigma `images` documental (decisión 32); **IMG-1** schema+catálogo not-ready (`DB_VERSION=26`, decisión 33); **IMG-3a** consumo compartido + persistencia admisión (`DB_VERSION=27`, decisión 34); **IMG-3b** attach canónico Application/AJAX (decisión 35); **IMG-4** lectura/sign-read (decisión 36); **IMG-5** etapa de eliminaciones (incs. 1–5) **cerrada** (`DB_VERSION=31`, decisión 41; plan §9); **§§10–11** exploración; **§12 Paso 1** declarativo **implementado** (decisión 44); **§13 Paso 2** picker/attach/presentación (decisión 45; **implementado**); **Paso 5 flip** (decisión 46; `is_ready=true`, `DEFAULTS_VERSION=3`); galería pendiente. Shell familias → `aa_canonical_*` + capabilities. Distinguir: familia `finance` (canónica) ≠ módulo legacy retirado. Rollback DB22 histórico: `docs/plans/canonical-shell-db22-rollback.md`.

- `canonical`: implementación operativa del contexto canónico. Dominio con registries sellados de familias, capabilities y solutions; `contact_dossier` se lee y muta mediante su application por lista, wire de solution separado y efecto neutral transaccional. Infraestructura: `AA_Canonical_Schema` `DB_VERSION=37`, applications y relación tipada de Expediente; capacidades y solutions no comparten tablas de configuración. El shell administrativo permanece en `includes/admin/ui/modules/canonical_shell/`; la ruta antigua `module=canonical` → 404.

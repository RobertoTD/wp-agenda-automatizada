...
- `canonical`: núcleo canónico de producto (Ciclo 1 + Ciclo 2A); dominio en `includes/domain/canonical/` (`AA_Canonical_Key`, `AA_Canonical_Family_Definition`, `AA_Canonical_Variant_Definition`, `AA_Canonical_Registry` instanciable y sellable); aplicación en `includes/application/canonical/` (`ResolveCanonicalRouteUseCase` para validación y resolución de ruta pura sin WordPress); infraestructura en `includes/infrastructure/canonical/` (`AA_Canonical_Core_Bootstrap` para composición y shared instance de `finance.general`) e `includes/infrastructure/wp/` (`AA_Canonical_Shell_Url_Policy` para URLs seguras allowlisted); UI en `includes/admin/ui/modules/canonical/` y layout ligero `includes/admin/ui/shared/canonical-layout.php`. Transporte vía `module=canonical&family=X[&variant=Y]`. Sin persistencia, CRUD ni tablas financieras.

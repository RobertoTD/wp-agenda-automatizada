...
- **Ruta vigente (PCU y continuación del shell):**
  ...
  10. **SB1-5B4** — delete record productivo: **implementado en working tree (sin commit).** Endpoint `aa_delete_canonical_record` + `WriteCanonicalShellRecordUseCase::delete`; UI «Eliminar» + modal de confirmación separado; hard delete + touch atómico; uncertain bloquea retry y ofrece «Recargar lista»; CRUD de records completo; harness nav actualizado a PCU-5B; sin soft delete ni edit/delete de contenedores; cero legacy.
  11. **SB1-5B+** — shell visual base restante: edit/delete de contenedores.
  12. **SET-1** — activación de familias/presets desde Settings (parcialmente anticipado por PCU-5A enablement AJAX; presets/capabilities siguen fuera).
  13. **CAP-1 / CAP-2 / CAP-3** — sistema de capabilities; `monetary_amount`; agregado monetario; imágenes.
  14. **LEGACY-X** — proyección, integración o deprecación selectiva de módulos legacy.

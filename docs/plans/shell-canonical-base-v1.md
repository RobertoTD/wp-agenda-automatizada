...
- **Ruta vigente (PCU y continuación del shell):**
  ...
  12. **SB1-5B6** — delete container productivo (hard delete + FK CASCADE): **implementado en working tree (sin commit).** Endpoint `aa_delete_canonical_container` + `WriteCanonicalShellContainerUseCase::delete`; command tag `[invalid_container_id]`; UI «Eliminar» + modal de confirmación separado; advertencia genérica sin conteo/preflight; una sola DELETE del contenedor; records vía `ON DELETE CASCADE`; uncertain bloquea retry y ofrece «Recargar listas»; CRUD universal de containers completo; sin soft delete; sin support PHP compartido; cero legacy.
  13. **SB1-5B+** — shell visual base restante (cerrado el CRUD contenedores; siguientes mejoras visuales/presets fuera de este cierre).
  14. **SET-1** — activación de familias/presets desde Settings (parcialmente anticipado por PCU-5A enablement AJAX; presets/capabilities siguen fuera).
  15. **CAP-1 / CAP-2 / CAP-3** — sistema de capabilities; `monetary_amount`; agregado monetario; imágenes.
  16. **LEGACY-X** — proyección, integración o deprecación selectiva de módulos legacy.

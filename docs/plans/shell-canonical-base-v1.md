...
- 40. **IMG-5 incremento 4 — retiro de lista (diseño)** — **diseño documentado** (2026-09-14) en el plan §7; **sin código**. Reutiliza captura `scope=container`, overlap, HMAC e intención del 1–3. Sustituye `aa_delete_canonical_container` shell por orquestación con seal obligatorio (incl. inventario vacío `expected_batch_count=0`), fail-closed RESTRICT, chunks locales post-sello si el presupuesto no basta, Continuar en modal de lista. Retiro de **una imagen** (§8 del plan) es incremento aparte. **Excluido aún:** implementación, `is_ready`, worker, Expedientes.

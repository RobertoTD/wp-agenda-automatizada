...
- 20. **Navegación Listas + filtro header + default create (Ciclo 2A UI)** — sidebar «Listas» activo en todo `canonical_shell` (visible con acceso al módulo aunque 0 familias); filtro header «Todas las listas» + familias enabled (`aria-label="Filtrar listas"`; current = contexto mostrado, no `lists_scope`); 0 familias → título estático; ≥1 → disclosure siempre; Settings toggles sin regeneración live de nav/sidebar (`postMessage`/DOM nav retirados; refresh por navegación/recarga); política de familia inicial al crear solo en JS (`resolveInitialCreateFamilyKey`: filtrada / archive preferida / N=1 / elección explícita); Ciclo 2B (FAB + retirada encabezado interno) pendiente.

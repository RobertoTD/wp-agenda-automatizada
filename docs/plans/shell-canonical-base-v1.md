...
- **Ruta vigente (PCU y continuación del shell):**
  ...
  9. **SB1-5B3** — update record productivo: **implementado en working tree (sin commit).** Endpoint `aa_update_canonical_record` + `WriteCanonicalShellRecordUseCase::update`; UI «Editar» + modal create/update unificado (`canonical-shell-record-form.js`); command endurecido (200/details null); last-write-wins; preserva `public_id`/`created_at`/`container_id`; tx UPDATE + touch parent; redirect página 1 de records sin `page`/`containers_page`; sin delete ni edit/delete de contenedores; sin optimistic lock; cero legacy.
  10. **SB1-5B+** — shell visual base restante: delete de records; edit/delete de contenedores.
  11. **SET-1** — activación de familias/presets desde Settings (parcialmente anticipado por PCU-5A enablement AJAX; presets/capabilities siguen fuera).
  12. **CAP-1 / CAP-2 / CAP-3** — sistema de capabilities; `monetary_amount`; agregado monetario; imágenes.
  13. **LEGACY-X** — proyección, integración o deprecación selectiva de módulos legacy.

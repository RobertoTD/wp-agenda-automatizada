...
- **Ruta vigente (PCU y continuación del shell):**
  ...
  16. **CAP-*** — … **Paso 1 activación por lista** (decisión 44 / plan §12.I): **preparación declarativa implementada**. **Paso 2 picker/attach/presentación mínima** (decisión 45 / plan §13): **propuesta docs pendiente de autorización**. Pendiente: implementar §13 → flip `is_ready` + bump `DEFAULTS_VERSION`; galería completa; residuales Continuar/Cerrar/banner; worker ops.
...
- 45. **Paso 2 — picker, subida post-save y presentación mínima (solo docs)** — **propuesta pendiente de autorización** (`canonical-images-retire-wp-integration.md` §13). Servidor attach/sign/contributor listos; falta módulo shell images + hook post-save antes de redirect + thumb mínima (última por `id DESC`). Sin galería; sin flip ready; stubs para desarrollo; producto requiere readiness conjunto. HEAD base: `82bf1d9cc1b5edfd74b6e5836c09b7bf8206a912`.

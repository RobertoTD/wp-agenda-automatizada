...
- **Ruta vigente (PCU y continuación del shell):**
  ...
  16. **CAP-*** — sistema de capabilities y `amount` (clave estable aprobada). **Histórico / supersedido como nombre normativo:** el plan nombraba `monetary_amount` para el importe; la clave vigente es `amount` (ver decisión 26 y `docs/05-canonical-capabilities.md`). **C1a completada** (decisión 27): schema/config/catálogo con `amount` no ready. Pendiente: A1a escritura de valores; A1b lectura/UI + ready; luego agregado monetario e imágenes. Secuencia: documentación (D0) → infra C1a → `amount` operable → transición/retirada Finance → imágenes.

...
- 25. **Opciones en encabezado + geometría de menú (Ciclo 5 UI)** — opciones ⋮ solo con registro abierto, en `.aa-shell-record-header` (hermanas del toggle, no anidadas); reserva `pr-12` en el toggle vía `:has(.aa-shell-record-options)` (clickeable al cerrado; sin cambio de altura/`break-words`); menú hermano del wrap con `w-[12rem] max-w-full min-w-0 box-border` (sin `min-w-[12rem]`); búsqueda del menú y clic fuera = `.aa-shell-record-options` **o** `.aa-shell-record-options-menu`; `overlayBottom = max(panel, menú visible)`; contorno continuo header+panel solo con `:has(.aa-shell-record-toggle:focus)` al abierto; Escape/CRUD/preview/fill intactos.

...
- 41. **IMG-5 incremento 5 — retiro de una imagen (registro vivo)** — **diseño aceptado, no implementado**. Alcance `scope=image`, `target_id=image.id`, `record_id` durable en purge_runs (`DB_VERSION=31` previsto); captura puntual por `upload_operation_id`; reutiliza `CanonicalPurgeRemoteMandateAdvancer` (accept + seal count=1); TX local solo image+op; registro y demás capabilities intactos; sin `is_ready`; sin cambio backend. Plan: `docs/plans/canonical-images-retire-wp-integration.md` §8.

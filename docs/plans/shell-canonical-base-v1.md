...
- 44. **Paso 1 — activación por lista `images` (solo docs)** — **propuesta pendiente de autorización** (`canonical-images-retire-wp-integration.md` §12). Delta mínimo: seeds 4 familias + label «Imágenes» + bump `DEFAULTS_VERSION` acoplado al flip `is_ready` (no bump prematuro). Checkbox vía montaje común (sin módulo JS images). Validación Paso 1 = AC/stubs aislados; **no** UI producto mientras not-ready. Resume/confirm tras desactivar: pendiente de matizar en paso de subida; no cambia en Paso 1. HEAD base: `33e4bd886e2e29d2538e1be4193e22a69a628214`.

...
- **Ruta vigente (PCU y continuación del shell):**
  ...
  6. **PCU-5** — dividido en **PCU-5A** / **PCU-5B**:
     ...
     - **PCU-5B** — bindings universales, fail-on-duplicate, desconexión Finance legacy del shell: **implementado en working tree (sin commit).** Shell `finance.general` / `archive.general` (enabled) → `AA_Canonical_Relational_Read_Adapter` + `aa_canonical_*`; bootstrap write universal listo (`AA_Canonical_Write_Binding_Bootstrap`) **sin** consumidor CRUD productivo; módulo clásico Finanzas sigue en `aa_finance_*`; cero dual-read/write, fallback o migración.

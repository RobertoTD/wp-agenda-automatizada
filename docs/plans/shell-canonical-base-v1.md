...
- SB1-4A (timestamps autoritativos en persistencia Finance): columna `updated_at datetime NOT NULL` en `aa_finance_containers` y `aa_finance_records`; `DB_VERSION=20`; índices canónicos `(variant_key, updated_at, id)` y `(container_id, updated_at, id)`; autoridad única `current_time('mysql')` en repositorios (interpretación futura con `wp_timezone()` antes de `AA_Canonical_Instant` UTC en SB1-4B); actividad de registros actualiza `updated_at` del contenedor padre en la misma transacción; sin exposición en DTOs/AJAX/UI. **Estado: implementado en working tree (sin commit).**
- SB1-4B (adaptador Finance legacy + binding `finance.general`): **pendiente**.
- Siguiente ciclo tras SB1-4B: pasos 6–7 de la ruta progresiva (conectar `finance.general` vía adaptador real; cerrar prueba arquitectónica).

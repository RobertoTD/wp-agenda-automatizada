...
Hoy (tras IMG-5 inc. 4 / DB 30): schema `DB_VERSION=30`; repertorio familiar en `aa_canonical_family_capabilities`; selección explícita de capacidades por lista en create/update del shell; familia canónica `finance` y capability `amount` (`is_ready=true`) sobre `aa_canonical_*`; escritura/lectura/UI amount operativas; único normalizador `AA_Canonical_Amount_Normalizer`; tablas físicas `images`/ops/purge + inventario de captura de retiro + checkpoints de retiro de lista; helper de consumo de instalación; attach Application/AJAX canónico (`aa_attach_canonical_record_image`) implementado; cliente HMAC de mandatos + captura durable de purge (IMG-5 inc. 1–2); retiro de un registro y de una lista vía mandatos (`aa_delete_canonical_record` / `aa_delete_canonical_container`); **retiro de una imagen (inc. 5) diseñado, no implementado** (`docs/plans/canonical-images-retire-wp-integration.md` §8); capability `images` **registrada `is_ready=false`** (sin seeds ni UI de producto). Validación integrada policyytest 2026-09-14: retiro de registro vacío y cancelación por conflicto **PASS** por AJAX; attach HTTP `canonical_v1` + retiro de registro con imagen **PASS** tras parche de `authorize-upload` (camino de desarrollo acotado); UI Eliminar vacío + Cancelar conflicto acreditados (Continuar de registro no PASS); incremento 4 AJAX A/B/C **PASS**; Continuar visual de lista desde listado **PASS** (lista 22); Cerrar/recargar/banner del 4 pendientes; lista 21 primer intento no PASS. El módulo Finance legacy (`module=canonical`, `aa_finance_*`) está **retirado**.
...
- **IMG-5 inc. 5:** diseño (§8 del plan de retiro) — `scope=image`, captura puntual, seal count=1, TX local solo de la asociación; **no implementado**; `is_ready` sigue false. Retiro de una imagen pendiente de código.

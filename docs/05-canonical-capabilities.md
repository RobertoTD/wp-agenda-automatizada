...
- **IMG-5 inc. 4:** `DB_VERSION=30`; `RetireCanonicalContainerUseCase` + `aa_delete_canonical_container`; seal obligatorio (incl. count=0); chunks keyset post-sello; modal de lista. AC aislados PASS. Prueba manual no ejecutada. Retiro de una imagen (§8 del plan) pendiente aparte.
